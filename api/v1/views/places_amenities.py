#!/usr/bin/python3
"""New view to link Place objects with Amenity objects."""
...
def amenities_from_place(place_id):
    """Access all amenities related to a Place object."""
...
def delete_amenity_from_place(place_id, amenity_id):
    """Delete an Amenity from a Place object using its ID."""
    place = storage.get(Place, place_id)
...
def insert_amenity_in_place(place_id, amenity_id):
    """Insert a new Amenity into a Place object."""
    place = storage.get(Place, place_id)
    amenity = storage.get(Amenity, amenity_id)
    if place is None or amenity is None:
        abort(404)
